...
export default function ProjectCard({ details }: ProjectCardProps) {
  ...
  return (
    <div
      className="relative w-[800px] h-[525px] border-white rounded-[10px] border-[10px] shadow-md"
      style={{ transform: `rotate(${rotation})` }}
      onClick={(e) => e.stopPropagation()}
    >
      <div className="relative w-full h-full flex flex-col justify-center items-center overflow-hidden">
        <div
          className={`w-full h-auto flex flex-row flex-1 bg-[var(--champagne-pink)] pt-[100px] px-[25px] gap-x-[25px] ${
            styles.pattern
          } ${details.class ? styles[details.class] : ""} `}
        >
          <div className="relative flex justify-center items-center w-[60%] h-full pb-[50px] z-[1]">
            <Image
              src={TextBlob}
              alt="text-box"
          ...
          <div className="flex flex-col justify-center items-center w-[40%] h-full gap-y-3 pb-[25px] z-[1]">
            ...
            <div className="flex flex-col gap-y-2">
              {details.role && (
                <p className="font-bold">Role: {details.role}</p>
              )}
        ...
        {details?.githubLink && (
          <Link href={details.githubLink} target="_blank">
            <div className="absolute flex flex-row justify-center items-center left-[10px] bottom-[10px] gap-x-[20px]">
              <Image
                src={GithubIcon}
                alt="github-link"
